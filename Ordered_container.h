...
In these functions, the container parameter is declared const when the container
will not be modified by the function. A data pointer is declared const if its value
is used only for comparison purposes and will not be modified nor stored in the container.
Likewise, an item pointer is declared const only if it is used as an input to a comparison.
